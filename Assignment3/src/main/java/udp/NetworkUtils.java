...
/**
 * NetworkUtils class provides logic to send and receive messages between client and server.
 *
...
// https://mkyong.com/java/java-convert-byte-to-int-and-vice-versa/
  ...
  // collecting packets as long as the size of the packets is smaller than the total ones we are supposed to receive
  public static Tuple Receive(DatagramSocket sock) throws IOException {
    ...
    // iterating through all packets, adding the payload of each package to the buffer, make sure offset is used to not overwrite things
    for(var p : packets) {
